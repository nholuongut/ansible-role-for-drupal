...
Drupal is a PHP-based application that is meant to run behind a typical LAMP/LEMP/LEPP/etc. stack, so you'll need at least the following:
  ...
  - Apache or Nginx (Recommended: `nholuong.apache` or `nholuong.nginx`)
  - MySQL or similar Database server (Recommended: `nholuong.mysql` or `nholuong.postgresql`)
  - PHP (Recommended: `nholuong.php` along with other PHP-related roles like `php-mysql`).

Drush is not an absolute requirement, but it's handy to have, and also required if you use this role to Install a Drupal site (`drupal_install_site: true`). You can use `nholuong.drush` to install Drush.

Git is not an absolute requirement, but is required if you're deploying from a Git repository (e.g. `drupal_deploy: true`). You can use `nholuong.git` to install Git.
